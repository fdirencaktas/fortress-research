# Fortress Research

## About
This repository contains the code and data (where permissible) for my research papers on quantitative finance, political economy of markets, and systematic trading.

## Author
Fikri Direnç Aktaş
- [LinkedIn](www.linkedin.com/in/direncaktas)
- [SSRN](https://papers.ssrn.com/sol3/cf_dev/AbsByAuth.cfm?per_id=11237428)

## Repository Structure


fortress-research/
│

│

├── README.md

├── LICENSE

├── requirements.txt


## Papers

| # | Title | SSRN Link | Code |
|---|-------|-----------|------|
| 1 | Two Primitives of Spot Market Strategy | [https://papers.ssrn.com/abstract=6736280] | 
| 2 | Two Primitives, One Synthetic | [https://papers.ssrn.com/abstract=6614880] | 
| 3 | Performance and Risk Analysis... (Thesis) | (Forthcoming) | [[code](https://github.com/fdirencaktas/fortress-bist-momentum/blob/main/src/fortress_bist_backtest.py)]
| 4 | The Political Economy of Market Regimes (Forthcoming)
| 5 | A Decision Framework for the Political Systematical Trader (Forthcoming)
| 6 | Are Turkish REITs Proxies for Direct Real Estate? | (Forthcoming) | [[code](https://github.com/fdirencaktas/fortress-research/blob/main/reit_arfima_dcc/code/code.py)] | 
| 7 | Strategic Unreasonableness: A Tool of the Political Systematical Trader | (Forthcoming) | [[code](https://github.com/fdirencaktas/fortress-research/blob/main/strategic-unreasonableness/code/Strategic_Unreasonableness.py)]
